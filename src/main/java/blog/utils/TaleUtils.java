...
public class TaleUtils
{
    ...
    public static void updateJDBCFile(String url, String dbName, String userName, String password)
    {
        ...
        try
        {
            ...
            properties.setProperty("spring.datasource.dbname", dbName);
            ...
            properties.store(fos, "update jdbc info");

        } catch (FileNotFoundException e)
        {
            LOGGER.info("updateJDBCFile method fail:{}", e.getMessage());
            e.printStackTrace();
        } catch (IOException e)
        {
            LOGGER.info("updataJDBCFile method fail:{}", e.getMessage());
            e.printStackTrace();
        } finally
        {
            if (fos != null)
     ...
     * 获取properties的配置文件
     *
     * @param fileName 举例：application-jdbc.properties中的jar
     * @return
     */
    public static Properties getPropFromJar(String fileName)
    {
        ...
        if (resourceAsStream == null) throw new TipException("get resource from path fail");
        try
        {
            properties.load(resourceAsStream);
        } catch (IOException e)
        {
            LOGGER.info("ger properties file fail:{}", e.getMessage());
            e.printStackTrace();
        }
        return properties;
    }
    ...
    public static DataSource getNewDataSource()
    {
        if (newDataSource == null) synchronized (TaleUtils.class)
        {
            if (newDataSource == null)
            {
                ...
                if (properties.size() == 0) return newDataSource;
    ...
    public static Integer getCoolieUid(HttpServletRequest request)
    {
        if (request != null)
        {
    ...
    {
        Cookie[] servletCookie = request.getCookies();
        if (servletCookie == null) return null;
        for (Cookie c : servletCookie)
        {
    ...
    public static String mdToHtml(String markdown)
    {
        ...
        String content = renderer.render(docment);
