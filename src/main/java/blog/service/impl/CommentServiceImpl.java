...
public class CommentServiceImpl implements CommentService
{
    ...
    {
        if (commentVo == null) throw new TipException("评论对象为空");
        if (StringUtils.isBlank(commentVo.getAuthor())) commentVo.setAuthor("热心网友");
        if (StringUtils.isNotBlank(commentVo.getMail()) && !TaleUtils.isEmail(commentVo.getMail()))
            throw new TipException("请输入正确的邮箱");
        if (StringUtils.isBlank(commentVo.getContent())) throw new TipException("评论内容不能为空");
        if (commentVo.getContent().length() < 5 || commentVo.getContent().length() > 2000)
            throw new TipException("评论字数控制在5-2000字数");
    ...
    public PageInfo<CommentBo> getComments(Integer cid, int page, int limit)
    {
        if (cid != null)
        {
            ...
            List<CommentVo> parents = commentDao.selectByExample(commentVoExample);
    ...
    @Override
    public PageInfo<CommentVo> getCommentWithPage(CommentVoExample commentVoExample, int page, int limit)
    {
        PageHelper.startPage(page, limit);
